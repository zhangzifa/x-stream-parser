...
const bytesUint16 = 2;
const bytesUint32 = 4;
const supportedBytes = [bytesFixed, bytesUint8, bytesUint16, bytesUint32];
...
function Parser(sync, bytes_payload_in_byte, max_payload_length) {
  ...
  this.rcvd = false;
  this.fixedLength = 0;
  this.maxPayloadLength = max_payload_length || -1;
  this.buffers = [];
  this.data = null;
  this.extra = null;
  this.timer = null;
...
Parser.prototype.append = function(data) {
  ...
  if (this.bytesPayloadInByte === bytesUint32) {
    if (this.maxPayloadLength !== -1) {
      this.bytesPayload = buf.readUInt16BE(headerIndex + this.bytesSync + 2);
    } else {
  ...
  } else if (this.bytesPayloadInByte == bytesUint8) {
    this.bytesPayload = buf.readUInt8(headerIndex + this.bytesSync);
  } else {
    this.bytesPayload = this.fixedLength;
  }
...
Parser.prototype.setFixLength = function(l) {
  this.fixedLength = l;
};
